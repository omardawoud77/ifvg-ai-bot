    ...
    def _on_step(self):
        if self.n_calls % self.eval_every == 0:
            ...
            if pnl > self.best_pnl:
                self.best_pnl = pnl
                self.steps_since_best = 0
                self.model.save("rl_agent_v2_120d_best")
...
model = PPO(
    ...
    gae_lambda=0.95,
    clip_range=0.2,
    ent_coef=0.001,  # low entropy = exploit what works
    verbose=0,
...
best = PPO.load("rl_agent_v2_120d_best")
...
print(f"\n💾 Saved: rl_agent_v2_120d_best.zip")
